...
LOCAL_GRID_JSON = os.path.join(BASE_DIR, "data/json/local_grid_id.json")
IND_PATTERN = os.path.join(BASE_DIR, "data/YEOSU_IND_CODE.txt")
...
def process_kcb(logger):
    ...
    logger.info(f"KCB 파일 목록: {kcb_files}")
    logger.info(f"업종코드 파일 목록: {ind_files}")
    ...
    logger.debug(f"Reading KCB file: {kcb_file}")

    kcb = pd.read_csv(kcb_file, sep='|')
    ind_code = pd.read_csv(ind_file, sep='|')
    logger.debug(f"KCB df shape: {kcb.shape}, IND_CODE df shape: {ind_code.shape}")
    ...
    logger.debug(f"Final KCB DataFrame head:\n{kcb.head()}")
...
def process_local2(logger):
    ...
    logger.debug(f"Reading Local Pay file: {pay_file}")
    local_pay = pd.read_csv(pay_file)
    logger.debug(f"Initial Local Pay RAW df shape: {local_pay.shape}")
    ...
    logger.debug(f"Final Local Pay RAW DataFrame head:\n{local_pay.head()}")
    # DB 적재
    engine = get_engine_from_env()
    local_pay['reg_dttm'] = datetime.now()
    ...
    logger.info("✅ Local Pay RAW 데이터 DB 적재 완료")
...
if __name__ == "__main__":
    ...
    logger.debug(f"  - KCB_PATTERN: {KCB_PATTERN}")
    logger.debug(f"  - LOCAL_PAY_PATTERN: {LOCAL_PAY_PATTERN}")
    logger.debug(f"  - LOCAL_GRID_JSON: {LOCAL_GRID_JSON}")
    logger.debug(f"  - IND_PATTERN: {IND_PATTERN}")
