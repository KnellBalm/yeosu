...
-- KCB 집계 테이블
CREATE TABLE IF NOT EXISTS public.tb_kcb_stat(
    grid_id                CHAR(8) NOT NULL,
    ...
    tot_sales_amt5m_cnt    INTEGER,
    reg_dttm               TIMESTAMP
);
-- 날짜 캐스팅 인덱스 (기간 비교를 위한 WHERE 절에서 활용)
CREATE INDEX IF NOT EXISTS idx_tb_kcb_bs_yr_mon_date
    ON public.tb_kcb((to_date(bs_yr_mon, 'YYYYMM')));

-- 지역화폐 집계 테이블
CREATE TABLE IF NOT EXISTS public.tb_local_pay_agg (
    grid_id                 CHAR(8) NOT NULL,    
    std_ym                  TIMESTAMP,  -- YYYYMM 형식
    ind_type                VARCHAR(50),
    pay_cnt                 INTEGER,
    pay_amt                 INTEGER,
    reg_dttm                TIMESTAMP
...
CREATE INDEX IF NOT EXISTS idx_tb_local_pay_std_ym
    ON public.tb_kcb((to_date(std_ym, 'YYYYMM')));

-- 인구
CREATE TABLE IF NOT EXISTS yeosu_dm.tb_population (
    grid_id         CHAR(8) NOT NULL,
)